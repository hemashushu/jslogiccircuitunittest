...
也可以将一个多位位宽的端口拆分（只提取其中的一部分），假如 Out 是一个 32 位位宽的输出端口，如果只想提取低 8 位数据，可以这样写：

`Out[7:0]`

范围值先写高位，然后再写低位，中间用冒号分隔。也可以一次提取多个部分，实现重组，比如：
...
使用双引号把字符或者字符串包围起来，测试程序会以 UTF-8 编码格式把字符或者字符串转换为相应的 Byte 数组，然后把 Byte 数组按 Little-endian 方式转换成二进制串。
